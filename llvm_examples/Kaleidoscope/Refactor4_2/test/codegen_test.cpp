...
TEST_F(CodegenTest, OptimizationPasses) {
    // test case: def opt_test(x) x + (2.0 + 3.0)
    // Optimization pass should fold the constant addition to produce (x + 5.0) in the IR, eliminating the separate 2.0 and 3.0 adds.
    std::vector<std::string> args = {"x"};
    auto proto = std::make_unique<PrototypeAST>("opt_test", std::move(args));

    auto varX = std::make_unique<VariableExprAST>("x");
    auto num2 = std::make_unique<NumberExprAST>(2.0);
    auto num3 = std::make_unique<NumberExprAST>(3.0);

    auto innerAdd = std::make_unique<BinaryExprAST>('+', std::move(num2), std::move(num3));
    auto outerAdd = std::make_unique<BinaryExprAST>('+', std::move(varX), std::move(innerAdd));

    auto function = std::make_unique<FunctionAST>(std::move(proto), std::move(outerAdd));
    ...
    // check that the optimized IR contains the folded constant 5.0 instead of separate 2.0 and 3.0 adds
    EXPECT_TRUE(irStr.find("5.0") != std::string::npos)
        << "Optimized IR should contain folded constant 5.0\n" << irStr;

    // fadd should only appear once in the optimized IR
    size_t faddCount = 0;
    ...
    EXPECT_EQ(faddCount, 1) << "IR should be optimized to a single fadd instruction\n" << irStr;
}
